...
def evaluate(A, size=100, learn_ratio = 0.9):
    ...
    for t, event in enumerate(events):
        ...
        pool_idx = event[3]
        # print(pool_idx)
        chosen, reward = A.choose_arm(T_learn + T_deploy, user, pool_idx)
        if random.random() < learn_ratio:
            G_learn += float(reward)
            T_learn += 1
            A.update(chosen, reward, user, pool_idx)
            learn.append(G_learn / T_learn)
        else:
            G_deploy += float(reward)
            T_deploy += 1
            deploy.append(G_deploy / T_deploy)
        if T_learn + T_deploy > 1000:
            break
    ...
    print(
        "{:<20}{:<10}{}".format(
            A.algorithm, round((G_deploy+G_learn) / (T_deploy+T_learn), 4), execution_time
        )
    )
