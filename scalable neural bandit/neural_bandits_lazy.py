...
def nurl_rbmle(contexts, A, bias, theta_n, good):
    ...
    for k in range(numActions):
        u_t[k] = f[k].float() + 0.4 *math.sqrt((torch.mm(torch.mm(g[k].t().float(), A), g[k].float())/float(M)))
    arm = np.argmax(u_t)
    # print(f'f[arm]: {f[arm]}, var[arm]: {u_t[arm]-f[arm]}')
    # End of TODO
    print('arm: ', str(arm), 'good: ', str(good))
...
for expInd in tqdm(range(numExps)):
	...
	# TODO: Update A_rbmle, b_rbmle
        ...
        C = 1
        qwwqw = LA.det(A_rbmle)
        if t%1==0 and qwwqw > (1+C)*LA.det(A_rbmle_last):
