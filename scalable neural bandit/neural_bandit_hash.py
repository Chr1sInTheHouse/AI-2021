...
def L(x, r, theta_now, m, lda, theta_0, d, l, g, Fx):
    ...
    indices = torch.randperm(len(x))[:1000] if len(x)>1000 else torch.range(0, len(x)-1)
    fx = model.forward(x[indices]) if len(x)>1000 else model.forward(x)
    model.zero_grad()
    ...
    ll = loss(fx.double(), r[indices].double()) if len(x)>1000 else loss(fx.double(), r.double())
